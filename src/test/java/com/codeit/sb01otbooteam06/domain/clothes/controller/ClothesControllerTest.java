...
class ClothesControllerTest {
  ...
  @Test
  @DisplayName("PATCH /api/clothes/{clothesId} - 옷 수정 성공 (이미지 포함)")
  void updateClothes() throws Exception {
    UUID clothesId = UUID.randomUUID();

    // ClothesUpdateRequest 생성 (필수 필드 포함)
    ClothesUpdateRequest updateRequest = new ClothesUpdateRequest(
        "테스트 옷",
        "TOP",
        List.of()  // 필요한 경우 attributes 넣기
    );
    ...
    given(clothesService.update(eq(clothesId), any(ClothesUpdateRequest.class),
        any(MultipartFile.class)))
        .willReturn(clothesDto);
    ...
    verify(clothesService, times(1))
        .update(eq(clothesId), any(ClothesUpdateRequest.class), any(MultipartFile.class));
  }
